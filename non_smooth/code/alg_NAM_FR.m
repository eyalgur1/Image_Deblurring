function [x_APB,y_APB,val_all,rlerr_all]=alg_NAM_FR(P,Xor,Xcon,B,center,sigma_w,sigma_e,N,pars,s,r)
...
total_iter_counter=0; % accumulative iteraion counter (inclusing the first u-update)
out_iter_counter=0; % outer iteraion counter (inclusing the first u-update)

e=ones(length(vecX),1); % vector of all ones for l1 prox
ze=zeros(length(vecX),1); % matrix of all zeros for l1 prox

%%
while total_iter_counter<N
    V=X;
    
    out_iter_counter=out_iter_counter+1;
    jk=s+1; % set the number of inner FISTA iterations
    num_iter_restart=floor(jk/r); % numner of inner iterations before restart
    in_iter_counter=0; % AG inner itearion counter
    ...
    %%%%%%%%%%%%%The x-step%%%%%%%%%%%%%
    while in_iter_counter<jk
        t=1;
        for ii=1:num_iter_restart

            total_iter_counter=total_iter_counter+1;% update the accumulative iteration counter
            in_iter_counter=in_iter_counter+1;
            vecX_prev=vecX;
            ...
            if mod(total_iter_counter,1000)==0
                X_out(:,:,:,total_iter_counter/1000+1)=X;
            end
            
            if in_iter_counter<jk
                % Compute the objective value
                val_all(total_iter_counter)=obj_value(AX,X,B,y,sigma_w,sigma_e,pars);
                
                % Compute the error
                %rlerr_all(iter)=norm(X-Xor,'fro')/aa;
                rlerr_all(total_iter_counter)=norm(X(:)-Xcon(:),'fro')/aa;
            end
            
            if total_iter_counter==N
                break
            end
