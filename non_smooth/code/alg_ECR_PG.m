function [x_APB,y_APB,val_all,rlerr_all]=alg_ECR_PG(P,Xor,Xcon,B,center,sigma_w,sigma_e,N,pars,s)
...
while iter<N
    ...
    while in_iter<s
        ...
        Xtrans=DC.*trans(X);
        G=2*real(itrans(conj(DC).*(Xtrans-Btrans)));       
        G=G(:);
        vecX=X(:);
        %G=G+2*(sigma_w^2)*(pars.lambda)*vecV; % only for l2 squared (comment otherwise)
        G=G+2*(sigma_w^2)*(pars.lambda)*(1-pars.alpha)*vecX; % only for elastic net (comment otherwise)
