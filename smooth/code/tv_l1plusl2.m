function [prox_sol,i]=tv_l1plusl2(X,lambda,t,N)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
while((i<N)&(count<5))
    ...
    D=project((2*t/(2*t+sigma))*X-(lambda/(2*t+sigma))*Lforward(R));
    Q=Ltrans(D);

    % Taking a step towards minus of the gradient
    P{1}=R{1}+(2*t+sigma)/(32*lambda)*Q{1};
    P{2}=R{2}+(2*t+sigma)/(32*lambda)*Q{2};
    ...
    fval=norm(project((2*t/(2*t+sigma))*X-(lambda/(2*t+sigma))*Lforward(P)),'fro')^2;
