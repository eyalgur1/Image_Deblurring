function [x_APB,y_APB,val_all,rlerr_all]=SPA_RSTLS(P,Xor,Xcon,B,center,sigma_w,sigma_e,N,pars)
...
for k=1:N
    ...
    Xtrans=DC.*trans(X);
    size(Xtrans);
    G=2*real(itrans(conj(DC).*(Xtrans-Btrans)));%+(pars.lambda)*2*X;
    
    % Compute the Lip. constant of the gradient of H with respect to variable X (without backtracking)
    L=2*max(max(abs(DC).^2));%+(pars.lambda)*2;
    
    % Gradeint step
    Z=X-(1/L)*G;
    
    % Prox step
    X=(L/(L+2*pars.lambda*(sigma_w^2)))*Z;
    if mod(k,1000)==0
        X_out(:,:,k/1000+1)=X;
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
